...
import type { MealLean } from "../types";
...
export default function EditMealModal({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const type = String(fd.get("type") || "") as MealType;
    const reaction = String(fd.get("reaction") || "") as Reaction;
    const place = (String(fd.get("place") || "").trim() || undefined) as string | undefined;
    const notes = (String(fd.get("notes") || "").trim() || undefined) as string | undefined;
    ...
    try {
      ...
      const m = json?.meal ?? {};
      const updated: MealLean = {
        _id: String(m._id ?? meal!._id),
        userId: String(m.userId ?? meal!.userId),
      ...
      setError(err?.message || "No se pudo actualizar el meal");
  ...
  async function handleDelete() {
    if (!meal) return;
    if (!confirm("¿Seguro que quieres eliminar este meal?")) return;

    setSubmitting(true);
    try {
      const res = await fetch(`/api/meals/${meal._id}`, { method: "DELETE" });
      const text = await res.text();
      if (!res.ok) throw new Error(text || "Error eliminando meal");
      onDeleted(meal._id);
      onClose();
    } catch (err: any) {
      setError(err?.message || "No se pudo eliminar el meal");
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-900">Editar meal</h2>
          <button
            onClick={onClose}
